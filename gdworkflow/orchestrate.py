...
def build_task_prompt(task: Task, project_dir: str = "") -> str:
    ...
    parts.append("")
    parts.append("1. **brainstorming** — Follow the brainstorming skill's 7-step checklist:")
    parts.append("   1. **Explore project context** — check files, docs, recent commits")
    parts.append("   2. **Ask clarifying questions via Discord** — one at a time (see below)")
    parts.append("   3. **Propose 2-3 approaches** — with trade-offs and your recommendation")
    parts.append("   4. **Present design** — scale each section to its complexity")
    parts.append("   5. **Write design doc** — save to `docs/design_docs/YYYY-MM-DD-<topic>-design.md` and commit")
    parts.append("   6. **Spec self-review** — check for placeholders, contradictions, ambiguity, scope")
    parts.append("   7. **Transition to implementation** — invoke writing-plans skill")
    parts.append("   Do NOT skip brainstorming even for seemingly simple tasks. Do NOT wait for explicit approval after presenting the design — proceed directly to writing-plans.")
    parts.append("2. **writing-plans** — Follow the writing-plans skill to create the plan:")
    parts.append("   1. Save to `docs/design_docs/plans/YYYY-MM-DD-<feature-name>.md`")
    parts.append("   2. Map out files before defining tasks — each file gets one clear responsibility")
    parts.append("   3. Write bite-sized tasks (each step = one action, 2-5 minutes)")
    parts.append("   4. Every plan starts with a header: goal, architecture, tech stack")
    parts.append("   5. Every task includes exact file paths, complete code, exact commands with expected output")
    parts.append("   6. Use TDD: write failing test first, run it, implement, run it, commit")
    parts.append("   7. No placeholders — no TBD, TODO, or vague steps. Every code block is filled in.")
    parts.append("   8. Self-review: check spec coverage, placeholder scan, type consistency")
    parts.append("   9. Proceed directly to subagent-driven-development — do not ask for approval")
    parts.append("3. **subagent-driven-development** — Execute the plan (no worktree needed):")
    parts.append("   1. Read the plan, extract all tasks with full text, create task list")
    parts.append("   2. For each task, in order:")
    parts.append("      a. Dispatch an implementer subagent with the full task text + context")
    parts.append("      b. If the implementer asks questions, answer them and re-dispatch if needed")
    parts.append("      c. The implementer implements, tests, commits, and self-reviews")
    parts.append("      d. Dispatch a spec compliance reviewer subagent")
    parts.append("      e. If issues found — implementer fixes, re-review")
    parts.append("      f. Dispatch a code quality reviewer subagent")
    parts.append("      g. If issues found — implementer fixes, re-review")
    parts.append("      h. Mark task complete")
    parts.append("   3. After all tasks: dispatch a final code reviewer for the full implementation")
    parts.append("   4. Create a DONE file in the worktree root with a summary")
    parts.append("   Important: Do NOT create worktrees — you are already in the right directory.")
    parts.append("")
    ...
    parts.append("Follow the brainstorming skill's question discipline:")
    parts.append("- Ask **ONE question at a time**. Wait for the answer before asking the next.")
    parts.append("- **Every question must include 2-3 options** with trade-offs and your recommendation. Do NOT ask open-ended questions — always propose choices.")
    parts.append("- Focus on: purpose, constraints, success criteria.")
    parts.append("- If the request spans multiple independent subsystems, flag it for decomposition before asking detailed questions.")
    if project_dir:
